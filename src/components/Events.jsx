...
const Events = () => {
  ...
  const EventCard = ({ title, date, time, icon, custom }) => (
    ...
    >
      <div className="w-20 h-20 md:w-24 md:h-24 mx-auto rounded-full flex items-center justify-center mb-6 md:mb-8 border-[3px] border-soft-gold/60 shadow-md relative z-10 transition-transform duration-700 group-hover:scale-110 p-[6px] overflow-hidden bg-white/70 dark:bg-transparent">
        <img 
           src={`${import.meta.env.BASE_URL}${icon}`}
           alt={`${title} Icon`}
           className="w-full h-full object-contain"
           style={{ filter: "url(#remove-checkerboard)" }}
        />
      </div>
